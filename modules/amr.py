...
def amr(filenames_array, Config, logger, output_folder, type, samples, amr_directory, cluster, scheduler, amrdb):
    if type == "PE":
        for file in filenames_array:
            ...
            amr_cmd = "ariba run --verbose --force %s %s %s %s/%s --tmp_dir /tmp/" % (amrdb, file, second_part, output_folder, os.path.basename(first_part))
            #keep_logging(mlst_cmd, mlst_cmd, logger, 'debug')
            if cluster == "cluster":
                job_prefix = "%s/%s" % (output_folder, os.path.basename(first_part))
                generate_cluster_jobs(amr_cmd, job_prefix, scheduler, Config, logger)
