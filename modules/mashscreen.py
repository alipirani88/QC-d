...
def mashscreen(filenames_array, Config, logger, output_folder, type, samples, mashscreen_directory, cluster, scheduler, mash_refseq_db):
    if type == "PE":
        for file in filenames_array:
            ...
            mash_cmd = "mash screen -w %s %s | sort -gr > %s/%s.tab" % (mash_refseq_db, file, mashscreen_directory, os.path.basename(first_part))
            #keep_logging(mlst_cmd, mlst_cmd, logger, 'debug')
            if cluster == "cluster":
                job_prefix = "%s/%s" % (output_folder, os.path.basename(first_part))
                generate_cluster_jobs(mash_cmd, job_prefix, scheduler, Config, logger)
